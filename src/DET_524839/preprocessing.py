...
def train_val_test_split(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    trv_df, te_df = train_test_split(df, test_size=TEST_SIZE, random_state=RANDOM_STATE, stratify=df["city"])
    tr_df, v_df = train_test_split(trv_df, test_size=VAL_SIZE, random_state=RANDOM_STATE, stratify=trv_df["city"])
